  ...
  build: {
    ...
    $pack: [
      [
        ...
        series.env('pack build', { ESNEXT: '#' }),
        copy('./static', './pkg/static')
      ],
    ...
    $node: series('babel ./src --out-dir ./pkg/dist --source-maps inline', {
      args: ['--extensions', vars.dotExt]
    }),
    $types: project.typescript && [
      `ttsc --project ttsconfig.json --outDir ./pkg/dist/`,
      copy(glob`./src/**/*.d.ts`, { from: 'src', to: 'pkg/dist' })
    ]
  },
